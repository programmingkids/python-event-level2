...
class Game():
    ...
    def start(self):
        ...
        self.firstShape = choice(self.SHAPES)
        self.secondShape = choice(self.SHAPES)
        ...
        self.frame = Frame(self.root, width=Game.SIDE, height=500, bg='orange')
    ...
    def timer(self):
        if self.create_new_game == True:
            self.firstShape = self.secondShape
            self.secondShape = choice(self.SHAPES)
            self.nextCanvas.destroy()
            self.nextCanvas = Canvas(self.frame,width=Game.SIDE,height=Game.NEXT_HEIGHT,bg='yellow')
            self.nextCanvas.pack(fill='both', side='top')
            for point in self.secondShape[1:]:
                if self.secondShape[0] is "kuroojyo":
                    self.image = self.kuroojyo
                elif self.secondShape[0] is "syogun":
                    self.image = self.syogun
                elif self.secondShape[0] is "murasaki":
                    self.image = self.murasaki
                elif self.secondShape[0] is "mushi":
                    self.image = self.mushi
                elif self.secondShape[0] is "tonakai":
                    self.image = self.tonakai
                elif self.secondShape[0] is "ryu":
                    self.image = self.ryu
                elif self.secondShape[0] is "akanin":
                    self.image = self.akanin

                self.nextCanvas.create_image(point[0] * Shape.BOX_SIZE + 40 + Shape.BOX_SIZE / 2,
                                          point[1] * Shape.BOX_SIZE + Shape.BOX_SIZE / 2 + 40,
                                          image=self.image)
            self.current_shape = Shape(self.canvas, self.kuroojyo, self.mushi, self.syogun, self.murasaki, self.tonakai, self.ryu, self.akanin, self.firstShape)
...
class Shape:
    ...
    def __init__(self, canvas, kuroojyo, mushi, syogun, murasaki, tonakai, ryu, akanin, shape):
        ...
        if shape[0] is "kuroojyo":
            self.image = kuroojyo
        elif shape[0] is "syogun":
            self.image = syogun
        elif shape[0] is "murasaki":
            self.image = murasaki
        elif shape[0] is "mushi":
            self.image = mushi
        elif shape[0] is "tonakai":
            self.image = tonakai
        elif shape[0] is "ryu":
            self.image = ryu
        elif shape[0] is "akanin":
            self.image = akanin
