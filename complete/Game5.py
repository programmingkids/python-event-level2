import tkinter
from tkinter import Canvas, Label, Tk, StringVar, PhotoImage
...
class Game():
    ...
    def timer(self):
        ...
        if self.create_new_game == True:
            self.current_shape = Shape(self.canvas, self.kuroojyo, self.mushi, self.tonakai, self.murasaki, self.syogun, self.ryu, self.akanin)
...
class Shape:
    ...
    def __init__(self, canvas, kuroojyo, mushi, master, murasaki, ojyo, ryu, akanin):
        ...
        if self.shape[0] is "kuroojyo":
            self.image = kuroojyo
        elif self.shape[0] is "master":
            self.image = master
        elif self.shape[0] is "murasaki":
            self.image = murasaki
        elif self.shape[0] is "mushi":
            self.image = mushi
        elif self.shape[0] is "ojyo":
            self.image = ojyo
        elif self.shape[0] is "ryu":
            self.image = ryu
        ...
        for point in self.shape[1:]:
            ...
            #     fill=self.color)
            #
            # print(point[0] * Shape.BOX_SIZE + Shape.START_POINT)
            # print(point[1] * Shape.BOX_SIZE)
            # print(point[0] * Shape.BOX_SIZE + Shape.BOX_SIZE + Shape.START_POINT)
            # print(point[1] * Shape.BOX_SIZE + Shape.BOX_SIZE)
