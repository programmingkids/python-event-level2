...
class Block:
    ...
    def __init__(self, canvas):
        ...
        self.image = BLOCK_IMAGES[self.block[0]]
        self.canvas = canvas

        for point in self.block[1:]:
            # ボックスを最初の位置に表示します
            box = canvas.create_image(
                    point[0] * BOX_SIZE + self.start_point_x + BOX_SIZE / 2,
    ...
    def can_move_box(self, box, x, y):
        ...
        if coords[1] + BOX_SIZE / 2 + y > WINDOW_HEIGHT: return False
        # boxの左上のX座標にxを加算した値が、0より小さい場合、移動不可
        if coords[0] - BOX_SIZE / 2 + x < 0: return False
        # boxの右下のX座標にxを加算した値が、ウィンドウの幅よりも大きい場合、移動不可
        if coords[0] + BOX_SIZE / 2 + x > WINDOW_WIDTH: return False

        # 移動先の座標を計算する
        new_x1 = (coords[0] + coords[0]) / 2 + x,
        new_y1 = (coords[1] + coords[1]) / 2 + y,
        new_x2 = (coords[0] + coords[0]) / 2 + x,
        new_y2 = (coords[1] + coords[1]) / 2 + y
